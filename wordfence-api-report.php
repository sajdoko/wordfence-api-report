 ...
 * Description:       Exposes key Wordfence security data via a secure REST API endpoint for external monitoring, such as in a Laravel CRM.
 * Version:           1.4.0
 * Author:            Sajmir Doko
...
function wordfence_api_add_admin_menu() {
    add_options_page(
        ...
        'manage_options',
        'wordfence-api-report',
        'wordfence_api_options_page_html'
    );
}
add_action('admin_menu', 'wordfence_api_add_admin_menu');
...
function wordfence_api_get_report_data( WP_REST_Request $request ) {
  ...
  $issue_counts = $wf_issues->getIssueCounts();
	...
	$scan_summary = array(
		...
		'issue_counts_by_severity' => array(
